...
class BackendClient: Performer {
    ...
    init(_ baseURLString: String) {
        ...
        self.backgroundManager = Alamofire.SessionManager(configuration: URLSessionConfiguration.background(withIdentifier: "io.fika.Fika.BackgroundManager"))
        self.backgroundManager.backgroundCompletionHandler = { NSLog("Background upload completed") }
    }
    ...
    func getRequestInfo(_ intent: Intent) -> RequestInfo? {
        ...
        switch intent {
        case let .addAttachment(streamId, attachmentId, attachment):
            info.endpoint = (.post, "/v\(API_VERSION)/streams/\(streamId)/attachments/\(attachmentId)")
            if let json = try? JSONSerialization.data(withJSONObject: attachment.data, options: []) {
                info.form["data"] = String(data: json, encoding: .utf8)
            }
        ...
        case let .getServiceGroups(service, teamId):
            info.endpoint = (.get, "/v\(API_VERSION)/services/\(service)/groups")
            info.queryString["team_id"] = teamId

        case let .getServiceUsers(service, teamId):
            info.endpoint = (.get, "/v\(API_VERSION)/services/\(service)/users")
            info.queryString["team_id"] = teamId
        ...
        case let .hideStream(id):
            info.endpoint = (.post, "/v\(API_VERSION)/streams/\(id)")
            info.queryString["visible"] = "false"

        case let .joinServiceGroup(identifier, autocreate):
            info.endpoint = (.post, "/v\(API_VERSION)/streams")
            info.queryString["autocreate"] = autocreate.description
            info.queryString["service_content_id"] = identifier.description
        ...
        case let .getStreamChunks(streamId):
            info.endpoint = (.get, "/v\(API_VERSION)/streams/\(streamId)/chunks")

        case let .sendChunk(streamId, chunk, persist, showInRecents, duplicate):
            ...
            info.queryString["allow_duplicate"] = duplicate.description
            info.queryString["persist"] = persist?.description
            if let contentId = chunk.externalContentId {
                info.queryString["export"] = "true"
                info.queryString["external_content_id"] = contentId
            }
            info.queryString["show_in_recents"] = showInRecents?.description
            if let attachment = chunk.attachments.first {
                // TODO: Support multiple attachments.
                // TODO: Consider allowing custom titles.
                if attachment.url.isFileURL {
                    info.form["attachment"] = attachment.url
                } else {
                    info.form["attachment_url"] = attachment.url.absoluteString
                }
            }
            info.form["payload"] = chunk.url
            if let segments = chunk.textSegments {
                info.form["text_segments"] = try! JSONSerialization.data(withJSONObject: segments.map { $0.data }, options: [])
            }
        ...
        case let .sendServiceInvite(service, teamId, identifiers):
            info.endpoint = (.post, "/v\(API_VERSION)/services/\(service)/invite")
            info.queryString["team_id"] = teamId
            info.form["identifier"] = identifiers

        case let .setChunkReaction(streamId, chunkId, reaction):
            info.endpoint = (.post, "/v\(API_VERSION)/streams/\(streamId)/chunks/\(chunkId)")
            info.queryString["reaction"] = (reaction ?? "")
    ...
    private func attemptFlushRetryQueue() {
        ...
        let taskId = UIApplication.shared.beginBackgroundTask (expirationHandler: {
            ...
            // TODO: End background task that was started earlier.
            NSLog("WARNING: Task ran out of time")
        })

        let retries = self.retryRequestQueue
        self.retryRequestQueue.removeAll()
        for intent in retries {
            NSLog("Retrying \(intent)")
            intent.perform(BackendClient.instance)
        }

        // TODO: Move this to after all intents have been performed.
...
struct RequestInfo: URLRequestConvertible {
    ...
    static var userAgentPieces: [(String, String)] = {
        ...
        return [
            ("Fika", Bundle.main.infoDictionary?["CFBundleVersion"] as? String ?? "UNKNOWN"),
    ...
    func asURLRequest() -> URLRequest {
        ...
        if self.authenticateClient {
            // Basic client auth.
            request.setValue("Basic ZmlrYTpjdkdtdkNoN0dnUENMM2Zpc2J2WnNtSHFsTXFsczlqVjJaS2VDMUlP", forHTTPHeaderField: "Authorization")
