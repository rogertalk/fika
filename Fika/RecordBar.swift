...
class RecordBar: UIView, ActionBarDelegate, RecordButtonDelegate {
    ...
    let actionButtonSize = CGSize(width: 50, height: 50)
    ...
    /// Whether the record button should visually appear as if it's recording. When true,
    /// audioLevel(for:) will be called every frame to show the audio level on the button.
    var isRecording = false {
        didSet {
            self.recordButton.isRecording = self.isRecording
        }
    }
    ...
    func action(for button: UIButton) -> ActionBar.Action? {
    ...
    func recordButton(_ recordButton: RecordButton, requestingState recording: Bool) {
        self.delegate?.recordBar(self, requestingAction: recording ? .beginRecording : .endRecording)
    }
